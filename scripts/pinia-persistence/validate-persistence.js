...
class PiniaPersistenceValidator {
    ...
    try {
      ...
      if (!analysis.hasPersistConfig && fileName === "user") {
        analysis.recommendations.push(
          "用户 store 应该持久化 token 和 userInfo",
        );
      }
      ...
      this.results.summary.errors.push(
        `分析文件 ${filePath} 时出错: ${error.message}`,
      );
      return null;
    }
  }
  ...
  generateReport() {
    ...
    this.results.stores.forEach((store) => {
      console.log(`\n🏪 ${store.file}.ts:`);
      console.log(
        `   持久化类型: ${this.getPersistTypeLabel(store.persistType)}`,
      );
  ...
  getStoreStatus(store) {
    if (store.persistType === "selective") {
      ...
      store.persistType === "full" &&
      store.temporaryStates.length > 0
    ) {
      return "⚠️ 需要优化";
  ...
  generateOverallRecommendations() {
    ...
    // 检查是否有全量持久化的 store
    const fullPersistStores = this.results.stores.filter(
      (s) => s.persistType === "full",
    );
