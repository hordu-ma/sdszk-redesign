...
class SimpleWorkflowValidator {
  ...
  checkWorkflowContent(content, fileName) {
    ...
    if (content.includes("actions/setup-node")) {
      ...
      // 提取Node版本
      const nodeVersionMatch = content.match(
        /node-version:\s*["']?([^"'\n]+)["']?/,
      );
      if (nodeVersionMatch) {
        ...
        if (analysis.nodeVersion !== "lts/*") {
          analysis.optimizations.push(
            `✅ 使用固定Node版本: ${analysis.nodeVersion}`,
          );
        }
      }
    }
  ...
  validate() {
    ...
    files.forEach((file) => {
      ...
      console.log(
        `   🎯 Gemini建议符合度: ${analysis.geminiCompliant ? "✅ 良好" : "⚠️ 可改进"}`,
      );
    ...
    console.log(
      `   符合Gemini建议的工作流: ${geminiCompliantCount}/${files.length}`,
    );

    const complianceRate = Math.round(
      (geminiCompliantCount / files.length) * 100,
    );
  ...
  checkGeminiRequirements(files) {
    ...
    console.log(
      `   ⚡ 性能优化配置: ${hasOptimizedConfig ? "✅ 已实现" : "❌ 缺失"}`,
    );
