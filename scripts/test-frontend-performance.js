...
class FrontendPerformanceTester {
  ...
  async analyzeBundleStructure() {
    ...
    try {
      ...
      const bundleTypes = {
        ...
        components: jsFiles.filter(f => !f.name.includes('vendor') && !f.name.includes('index')),
      };
  ...
  async analyzeRouteConfiguration() {
    ...
    try {
      ...
      // 检查同步导入的组件
      const syncImports = (routerContent.match(/^import\s+\w+\s+from\s+["']\.\.\/views/gm) || []).length;

      // 检查懒加载的组件
      const lazyImports = (routerContent.match(/component:\s*\(\)\s*=>\s*import\(/g) || []).length;

      // 检查预加载配置
      const hasPreloading = routerContent.includes('preloadComponents');

      // 检查性能监控集成
      const hasPerformanceMonitoring = routerContent.includes('startRouteTimer');
      ...
      this.testResults.routeAnalysis = {
        ...
        syncRatio: (syncImports / (syncImports + lazyImports) * 100).toFixed(1) + '%'
      };
  ...
  async checkOptimizations() {
    ...
    try {
      ...
      const optimizations = {
        hasGzipCompression: viteConfig.includes('viteCompression') && viteConfig.includes('gzip'),
      ...
      const hasPerformanceUtils = await fs.access(performanceUtilsPath).then(() => true).catch(() => false);
   ...
   */
  generateReport() {
    const { buildAnalysis, bundleAnalysis, routeAnalysis, optimizationChecks } = this.testResults;
    ...
    if (buildAnalysis.largestJSFile && buildAnalysis.largestJSFile.size > 1024 * 1024) {
