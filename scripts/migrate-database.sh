...
DUMP_DIR="./db-dump"
BACKUP_FILE="sdszk-backup-$(date +%Y%m%d-%H%M%S).gz"
...
# 2. 压缩导出文件
echo_success "步骤2: 压缩数据库文件..."
tar -czf $BACKUP_FILE -C $DUMP_DIR $LOCAL_DB
...
ssh $SERVER_USER@$SERVER_IP "
    ...
    # 备份现有数据库（如果存在）
    echo '备份现有数据库...'
    mongodump --db $REMOTE_DB --out ./backup-before-migration-\$(date +%Y%m%d-%H%M%S) 2>/dev/null || echo '没有现有数据库需要备份'
    ...
    # 恢复数据库
    echo '恢复数据库...'
    mongorestore --db $REMOTE_DB ./$LOCAL_DB/
