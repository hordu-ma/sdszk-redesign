...
// 静态方法：记录浏览历史
viewHistorySchema.statics.recordView = async function (
  userId,
  itemType,
  itemId,
  options = {},
...
// 静态方法：获取用户浏览历史
viewHistorySchema.statics.getUserHistory = async function (
  userId,
  options = {},
) {
  const {
...
// 静态方法：清理历史记录
viewHistorySchema.statics.clearUserHistory = async function (
  userId,
  options = {},
) {
...
// 静态方法：获取用户推荐内容
viewHistorySchema.statics.getRecommendations = async function (
  userId,
  options = {},
) {
  ...
  // 基于用户浏览历史的简单推荐算法
  const userHistory = await this.find({ user: userId })
    .sort("-lastViewedAt")
    .limit(20)
    .lean();
...
viewHistorySchema.pre("save", async function (next) {
  if (this.isNew) {
    const Model = mongoose.model(
      this.itemType === "news" ? "News" : "Resource",
    );
