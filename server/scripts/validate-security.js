  ...
  const securityChecks = [
    {
      name: "default-src 限制",
      check: () =>
        directives.defaultSrc?.includes("'self'") &&
        !directives.defaultSrc?.includes("*"),
      critical: true,
    ...
    },
    {
      ...
      description:
        "生产环境中 script-src 不应包含 unsafe-inline 或 unsafe-eval",
    },
    {
      name: "style-src 安全性",
      check: () => {
    ...
    },
    {
      ...
      check: () => {
        if (environment === "production") {
          return (
            !directives.imgSrc?.includes("https:") ||
...
function validateHelmetConfig(helmetConfig, environment) {
  ...
  const requiredHeaders = [
    ...
    },
    {
      ...
      check: () =>
        helmetConfig.referrerPolicy && helmetConfig.referrerPolicy.policy,
...
function generateReport(environment) {
  ...
  try {
    ...
    console.log("\n📋 CSP 配置验证:");
    console.log(
      `安全评分: ${cspResults.score}/${cspResults.maxScore} (${Math.round((cspResults.score / cspResults.maxScore) * 100)}%)`,
    );
    ...
    // 总体评估
    const hasErrors =
      cspResults.errors.length > 0 || helmetResults.errors.length > 0;
    const hasWarnings =
      cspResults.warnings.length > 0 || helmetResults.warnings.length > 0;
...
function main() {
  ...
  if (!["development", "production", "test"].includes(environment)) {
    console.error(
      "❌ 无效的环境名称。支持的环境: development, production, test",
    );
    process.exit(1);
  }
