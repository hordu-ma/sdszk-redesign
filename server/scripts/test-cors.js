...
async function runCorsSecurityTests() {
  ...
  // 测试允许的域名
  console.log("📝 测试允许的域名...");
  for (const origin of allowedOrigins.slice(0, 5)) {
    // 只测试前5个以节省时间
    for (const endpoint of TEST_CONFIG.endpoints) {
      ...
      if (preflightResult.success && requestResult.success) {
        ...
        console.log(
          `  ❌ ${origin} -> ${endpoint} (${preflightResult.error || requestResult.error})`,
        );
      }
    }
  }
  ...
  for (const origin of TEST_CONFIG.maliciousOrigins) {
    for (const endpoint of TEST_CONFIG.endpoints.slice(0, 2)) {
      ...
      const shouldBeBlocked =
        origin !== null && origin !== undefined && isProduction;
      const actuallyBlocked =
        !preflightResult.success || !requestResult.success;
      ...
      if (shouldBeBlocked && actuallyBlocked) {
        ...
        results.summary.passed++;
        console.log(
          `  ⚠️  ${origin || "no-origin"} -> ${endpoint} (开发环境允许)`,
        );
      } else {
        results.summary.failed++;
        console.log(
          `  🚨 ${origin || "no-origin"} -> ${endpoint} (安全问题！)`,
        );
      }
    }
  }
  ...
  const successRate = (
    ((results.summary.passed + results.summary.blocked) /
      results.summary.total) *
    100
  ).toFixed(1);
  ...
  // 检查是否有恶意请求成功
  const unblocked = results.maliciousOriginTests.filter(
    (test) => test.shouldBeBlocked && !test.actuallyBlocked,
  );
  ...
  // 检查允许的域名是否正常工作
  const blockedAllowed = results.allowedOriginTests.filter(
    (test) => !test.actuallyPassed,
  );

  if (blockedAllowed.length > 0) {
    securityIssues.push(
      `⚠️  ${blockedAllowed.length} 个允许的域名无法正常访问`,
    );
  }
  ...
  if (isProduction) {
    ...
    if (httpOrigins.length > 0) {
      securityIssues.push(
        `⚠️  生产环境存在 ${httpOrigins.length} 个 HTTP（非安全）域名`,
      );
    }
  }
