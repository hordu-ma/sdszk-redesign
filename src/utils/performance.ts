...
class PerformanceMonitor {
  ...
  private collectWebVitals() {
    ...
    // 这里处理降级情况或补充数据
    if (typeof window !== 'undefined' && !this.metrics.firstContentfulPaint && 'performance' in window) {
   ...
   */
  endRouteChange() {
    if (typeof window !== 'undefined' && 'performance' in window && this.routeStartTime > 0) {
  ...
  measureComponentLoad(componentName: string, startTime: number) {
    if (typeof window !== 'undefined' && 'performance' in window) {
      ...
      if (process.env.NODE_ENV === 'development') {
        console.log(`[Performance] Component '${componentName}' loaded in ${loadTime.toFixed(2)}ms`);
      }
    }
  }
