...
import type { FavoriteItem, FavoriteQueryParams, FavoriteStats } from '@/api/modules/favorite'
    ...
    response.data.favorites = response.data.favorites.filter(
      item =>
        ...
        (item.itemId.description &&
          item.itemId.description.toLowerCase().includes(keyword.toLowerCase())) ||
...
export const getFavoriteStats = async () => {
  ...
  return {
    ...
    data: {
      ...
      resources: response.data.byType.find((t: any) => t._id === 'resource')?.count || 0,
