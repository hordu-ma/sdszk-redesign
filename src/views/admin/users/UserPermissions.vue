<template>
  <div class="user-permissions">
      ...
      <div v-if="viewMode === 'tree'" class="tree-view">
        ...
        >
          <template #title="nodeData">
            <div class="permission-node" :class="{ 'is-leaf': !nodeData.children }">
              <div class="node-content">
                <div class="node-info">
                  <span class="node-name">{{ nodeData.displayName || nodeData.name }}</span>
                  <a-tag
              ...
              <div v-if="nodeData.id" class="node-actions">
                ...
                <a-button type="link" size="small" danger @click="deletePermission(nodeData)">
                  删除
                </a-button>
              </div>
            </div>
          ...
          <template #bodyCell="{ column, record }">
            ...
            <template v-else-if="column.key === 'actions'">
              <a-space>
                ...
                <a-button type="link" size="small" @click="viewPermissionUsage(record)">
                  查看使用
                </a-button>
      ...
      >
        <a-form-item label="权限名称" name="name">
          <a-input
            ...
            :disabled="isEditing"
          />
          <div class="form-tip">建议格式：module:action:resource，如：user:create:profile</div>
        </a-form-item>
        ...
        <a-form-item label="资源对象" name="resource">
          <a-input v-model:value="permissionForm.resource" placeholder="请输入资源对象" />
        </a-form-item>
    ...
    <!-- 权限使用情况模态框 -->
    <a-modal v-model:open="usageModalVisible" title="权限使用情况" :footer="null" width="700px">
      <div v-if="currentPermission" class="permission-usage">
        ...
        <div class="usage-section">
          ...
          <a-empty v-else description="暂无角色使用此权限" :image="simpleImage" />
        </div>
              ...
              <template #renderItem="{ item }">
                <a-list-item>
                  <a-list-item-meta>
                    ...
                    <template #description> {{ item.email }} - {{ item.role }} </template>
                  </a-list-item-meta>
                </a-list-item>
              </template>
            </a-list>
          </div>
          <a-empty v-else description="暂无用户拥有此权限" :image="simpleImage" />
        </div>
      </div>
    </a-modal>
  </div>
...
import type { PermissionItem, RoleItem, AdminUserItem } from '@/api/modules/adminUser'
...
const actionOptions = computed(() => {
  ...
  const allActions = Array.from(new Set([...commonActions, ...existingActions])).sort()
...
const getPermissionUsageCount = (permissionName: string) => {
  return roles.value.filter(role => role.permissions.includes(permissionName)).length
}
