...
const AvatarCropper = defineAsyncComponent(
  () => import("@/components/common/AvatarCropper.vue"),
);
...
const userInfo = computed(
  () => userStore.userInfo as unknown as ExtendedUserInfo,
);
...
const cropSuccess = async (blob: Blob, _dataUrl: string) => {
  ...
  try {
    ...
    const errorMessage =
      error instanceof Error ? error.message : "上传头像时发生错误";
...
const loadUserStats = async () => {
  try {
    ...
    const errorMessage =
      error instanceof Error ? error.message : "获取用户统计数据失败";
