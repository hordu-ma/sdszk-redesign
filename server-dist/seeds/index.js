...
const createNewsCategories = async () => {
  try {
    ...
    console.log(
      '新闻分类创建成功:',
      createdCategories.map(cat => ({ name: cat.name, key: cat.key, _id: cat._id }))
    )
  ...
  try {
    ...
    const sampleNews = [
      ...
      },
      {
        ...
        summary: '山东省思政课教师教学技能大赛将于2025年6月举行，欢迎各校教师积极参与。',
      ...
      },
      {
        ...
        summary: '本文件提出了加强山东省大中小学思政课一体化建设的具体措施和实施路径。',
    ...
    console.log(
      '成功创建示例新闻:',
      createdNews.map(news => ({ title: news.title, category: news.category }))
    )
    return createdNews
  } catch (err) {
...
const seedDB = async () => {
  try {
    ...
    // 删除所有现有数据
    await Promise.all([User.deleteMany({}), NewsCategory.deleteMany({}), News.deleteMany({})])
    console.log('已清空现有数据')
